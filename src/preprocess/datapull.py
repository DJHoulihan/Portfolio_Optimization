...
class DataHandler:
    ...
    def load_data(self):
        """
        Returns a dictionary with keys: 'train', 'val', 'test', each containing:
            {
                'obs': (T, N, K, F),
                'returns': (T, N)
            }
        ...
        df = pd.read_csv(project_root / self.config.data.data_path, delimiter=',')
        ...
        windows = np.stack([features[t-K:t] for t in range(K, T_raw)], axis=0)
        windows = temporal_zscore_normalize(windows)

        obs_windows = windows.transpose(0, 2, 1, 3)  # (T, N, K, F)
        returns = returns[K:]  # (T, N)
        ...
        data_splits = {
            'train': {'obs': obs_windows[:train_end], 'returns': returns[:train_end]},
            'val':   {'obs': obs_windows[train_end:val_end], 'returns': returns[train_end:val_end]},
            'test':  {'obs': obs_windows[val_end:], 'returns': returns[val_end:]}
        }

        # Convert all to float32
        for split in data_splits.values():
            split['obs'] = split['obs'].astype(np.float32)
            split['returns'] = split['returns'].astype(np.float32)

        print(f"[DataHandler] train: {data_splits['train']['obs'].shape}, val: {data_splits['val']['obs'].shape}, test: {data_splits['test']['obs'].shape}")
